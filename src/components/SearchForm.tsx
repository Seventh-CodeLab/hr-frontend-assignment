...
/* 
This solution only contains what was required for exercises 1 and 2. 
3, 4 and 5 are not present in this solution due to time constraints, but I will document here how I would solve them:

3: I would likely have solved task 3 by creating an async function to call the API provided, then return an array of Ship objects as "setSearchResult"
instead of simply putting down a string for what was searched.

4: To solve 4 I would attempt to call the form's submission function within SearchInput's onChange event.

5: In order to fix the issue of calling on every input change, I would likely try to implement Lodash debouncing into the API calling method
or alternatively try to implement timeouts between calls and cancel them if more input is made in the search box.
*/
...
const SearchForm = ({setSearchResult} : SearchFormProps) => {
    ...
    const Submit = (e: any) => {
        e.preventDefault() 
        e.stopPropagation() // Prevent the form submission event from reloading the HTML
        
        /* Code for performing a search */

        setSearchResult(`I searched for ${search}!!`) // Sample would normally likely be Json data
    }
